...
class _DAVResource(ABC):
    ...
    def __init__(self, path: str, is_collection: bool, scope: HTTPScope):
        assert util.is_str(path)
        assert path == "" or path.startswith("/")
        assert scope.asgidav.provider

        self.provider: DAVProvider = scope.asgidav.provider
        self.path: str = path
...
class DAVNonCollection(_DAVResource):
    ...
    def __init__(self, path: str, scope: HTTPScope):
        super().__init__(path, False, scope)
...
class DAVCollection(_DAVResource):
    ...
    def __init__(self, path: str, scope: HTTPScope) -> None:
        super().__init__(path, True, scope)
