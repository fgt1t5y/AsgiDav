...
class RequestServer:
    ...
    async def __call__(
        self, scope: HTTPScope, receive: ASGIReceiveEvent, send: ASGISendEvent
    ):
        assert scope.asgidav.verbose
        ...
        if scope.HTTP_DEPTH is not None:
    ...
    async def _send_response(
        self, scope: HTTPScope, send, root_res, success_code, error_list
    ):
        ...
        for refurl, e in error_list:
            ...
            responseEL = util.etree.SubElement(multistatusEL, "{DAV:}response")  # type: ignore
            util.etree.SubElement(responseEL, "{DAV:}href").text = refurl  # type: ignore
            util.etree.SubElement(  # type: ignore
                responseEL, "{DAV:}status"
            ).text = f"HTTP/1.1 {get_http_status_string(e)}"

        await util.send_multi_status_response(scope, send, multistatusEL)

    def _check_write_permission(self, res, depth, scope: HTTPScope):
        ...
        if not scope.asgidav.conditions._if:
            util.parse_if_header_dict(scope)

        # raise HTTP_LOCKED if conflict exists
        lock_man.check_write_permission(
            url=ref_url,
            depth=depth,
            token_list=scope.asgidav.ifLockTokenList,
            principal=scope.asgidav.user_name,
        )

    def _evaluate_if_headers(self, res, scope: HTTPScope):
        ...
        if_dict = scope.asgidav.conditions._if
        ...
        etag = util.checked_etag(res.get_etag(), allow_none=True)
        if etag is None:
            etag = "[]"  # Non-valid entity tag

        if (
            scope.HTTP_IF_MODIFIED_SINCE
            or scope.HTTP_IF_UNMODIFIED_SINCE
            or scope.HTTP_IF_MATCH
            or scope.HTTP_IF_NONE_MATCH
        ):
            util.evaluate_http_conditionals(res, last_modified, etag, scope)

        if not scope.HTTP_IF:
            return
        ...
        if lock_man:
            lockList = lock_man.get_indirect_url_lock_list(
                ref_url, principal=scope.asgidav.user_name
            )
    ...
    async def do_PROPFIND(self, scope: HTTPScope, receive, send):
        """
        ...
        if not scope.HTTP_DEPTH:
            scope.HTTP_DEPTH = "infinity"
        ...
        requestEL = await util.parse_xml_body(scope, receive, allow_empty=True)
        ...
        reslist = res.get_descendants(depth=scope.HTTP_DEPTH, add_self=True)
        ...
        if responsedescription:
            ...
            ).text = "\n".join(responsedescription)  # type: ignore

        await util.send_multi_status_response(scope, send, multistatusEL)

    async def do_PROPPATCH(self, scope: HTTPScope, receiv, send):
        ...
        for ppnode in requestEL:  # type: ignore
    ...
    async def do_MKCOL(self, scope: HTTPScope, receive, send):
        ...
        self._check_write_permission(parentRes, "0", scope)
    ...
    async def do_DELETE(self, scope: HTTPScope, receive, send):
        """
        ...
        if res.is_collection:
            ...
            if scope.HTTP_DEPTH not in ("0", "infinity"):
                self._fail(
                    HTTP_BAD_REQUEST,
        ...
        reverse_child_ist = res.get_descendants(
            depth_first=True, depth=scope.HTTP_DEPTH, add_self=True
        )
        ...
        if res.is_collection and res.support_recursive_delete():
            ...
            for child_res in reverse_child_ist:
                try:
                    self._evaluate_if_headers(child_res, scope)
                    self._check_write_permission(child_res, "0", scope)
                except Exception:
                    has_conflicts = True
                    break
        ...
        for child_res in reverse_child_ist:
            ...
            try:
                ...
                if provider.exists(child_res.path, scope):
                    raise DAVError(
    ...
    async def do_PUT(self, scope: HTTPScope, receive, send):
        ...
        if scope.HTTP_CONTENT_ENCODING:
    ...
    async def _copy_or_move(self, scope: HTTPScope, receive, send, is_move: bool):
        ...
        src_path = scope.path
        provider = self._davProvider
        src_res = provider.get_resource_inst(src_path, scope)
        src_parent_res = provider.get_resource_inst(
            util.get_uri_parent(src_path), scope
        )
        ...
        if not scope.HTTP_OVERWRITE:
            scope.HTTP_OVERWRITE = "T"

        if scope.HTTP_OVERWRITE not in ("T", "F"):
        ...
        if util.get_content_length(scope) != 0:
            ...
            _logger.info(f"Ignored copy/move  body: {body.getbuffer()[:50]!r}...")
        ...
        if src_res.is_collection:
            ...
            if is_move and scope.HTTP_DEPTH != "infinity":
                self._fail(
                    HTTP_BAD_REQUEST,
        ...
        # Windows quoted)
        http_destination = unquote(scope.HTTP_DESTINATION or "")
        ...
        fwd_scheme = scope.HTTP_X_FORWARDED_PROTO.lower()

        # hostnames are case-insensitive (#260)
        dest_netloc = dest_netloc.lower() if dest_netloc else ""

        url_host = scope.HTTP_HOST.lower()
        fwd_host = scope.HTTP_X_FORWARDED_PROTO.lower()
        ...
        dest_res = provider.get_resource_inst(dest_path, scope)
        dest_exists = dest_res is not None

        dest_parent_res = provider.get_resource_inst(
            util.get_uri_parent(dest_path), scope
        )
        ...
        self._evaluate_if_headers(src_res, scope)
        self._evaluate_if_headers(dest_res, scope)
        # Check permissions
        # http://www.webdav.org/specs/rfc4918.html#rfc.section.7.4
        if is_move:
            self._check_write_permission(src_res, "infinity", scope)
            # Cannot remove members from locked-0 collections
            if src_parent_res:
                self._check_write_permission(src_parent_res, "0", scope)

        # Cannot create or new members in locked-0 collections
        if not dest_exists:
            self._check_write_permission(dest_parent_res, "0", scope)
        # If target exists, it must not be locked
        self._check_write_permission(dest_res, "infinity", scope)
        ...
        if dest_exists and scope.HTTP_OVERWRITE != "T":
            self._fail(
                HTTP_PRECONDITION_FAILED,
        ...
        try:
            if is_move:
                ...
                isInfinity = scope.HTTP_DEPTH == "infinity"
        ...
        if is_move and src_res.support_recursive_move(dest_path):
            ...
            for s in src_list:
                try:
                    self._evaluate_if_headers(s, scope)
                except Exception as e:
                    _debug_exception(e)
                    has_conflicts = True
                    break
        ...
        for sres in src_list:
            ...
            try:
                ...
                self._evaluate_if_headers(sres, scope)
        ...
        await self._send_response(scope, send, src_res, success_code, error_list)

    async def do_LOCK(self, scope: HTTPScope, receive, send):
        """
        ...
        timeout_secs = util.read_timeout_value_header(scope.HTTP_TIMEOUT)
        submitted_token_list = scope.asgidav.ifLockTokenList

        lockinfo_el = await util.parse_xml_body(scope, receive, allow_empty=True)
        ...
        if lockinfo_el is None:
            ...
            scope.HTTP_DEPTH = "0"  # MUST ignore depth header on refresh
            ...
            lock_res = provider.get_resource_inst(lock_path, scope)
            ...
            await send(
                {
                    ...
                    "headers": [
                        ...
                        [b"Content-Length", util.to_bytes(str(len(xml)))],
                    ],
                }
            )
        ...
        if not lockinfo_el or lockinfo_el.tag != "{DAV:}lockinfo":
            self._fail(HTTP_BAD_REQUEST)
        ...
        lock_depth = scope.HTTP_DEPTH

        for linode in lockinfo_el:  # type: ignore
        ...
        await send(
            {
                "type": "http.response.start",
                "status": 201 if createdNewResource else 200,
                "headers": [
                    [b"Content-Type", b"application; charset=utf-8"],
                    [b"Date", util.to_bytes(util.get_rfc1123_time())],
                    [b"Content-Length", util.to_bytes(str(len(xml)))],
                    [b"Lock-Token", util.to_bytes(lock["token"])],
                ],
            }
        )
    ...
    async def do_UNLOCK(self, scope: HTTPScope, reveice, send):
        """
        ...
        if lock_man is None:
            ...
            self._fail(HTTP_NOT_FOUND, path)
        elif not scope.HTTP_LOCK_TOKEN:
            self._fail(HTTP_BAD_REQUEST, "Missing lock token.")

        self._evaluate_if_headers(res, scope)

        lock_token = scope.HTTP_LOCK_TOKEN.strip("<>")  # type: ignore
        ref_url = res.get_ref_url()
        ...
        await util.send_status_response(scope, send, HTTP_NO_CONTENT)

    async def do_OPTIONS(self, scope: HTTPScope, receive, send):
        """
        ...
        elif provider.is_collection(util.get_uri_parent(path), scope):
    ...
    async def _send_resource(self, scope: HTTPScope, receive, send, is_head_method):
        """
        If-Range
        ...
        elif scope.HTTP_DEPTH != "0":
            self._fail(HTTP_BAD_REQUEST, "Only Depth: 0 supported.")
        elif res is None:
            self._fail(HTTP_NOT_FOUND, path)
        elif res.is_collection:
            self._fail(HTTP_FORBIDDEN, "Directory browsing is not enabled.")
        ...
        if "response_headers" in scope.asgidav.config:
            custom_headers = scope.asgidav.config["response_headers"]
            for header, value in custom_headers:
                response_headers.append((header, value))

        res.finalize_headers(scope, response_headers)
    ...
    async def _send_head_resource(self, scope: HTTPScope, receive, send):
        """
        If-Range
            If the entity is unchanged, send me the part(s) that I am missing;
            otherwise, send me the entire new entity
