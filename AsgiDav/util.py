...
def to_set(val, *, or_none=False, raise_error=False) -> set | None:
    res = set()
    if type(val) is set:
        res = val
...
def get_dict_value(d, key_path, default=NO_DEFAULT, *, as_dict=False) -> dict | Any:
    ...
    if as_dict:
        try:
            res = get_dict_value(d, key_path, {})  # type: ignore
...
def check_tags(tags, known, *, msg=None, raise_error=True, required: Any = False):
...
def init_logging(config):
    ...
    **Note:** init_logging() is automatically called if an application adds
    ...
    """
    from AsgiDav.default_conf import DEFAULT_LOGGER_DATE_FORMAT, DEFAULT_LOGGER_FORMAT
...
def save_split(s: str, sep: str | None, maxsplit: int):
    """Split string, always returning n-tuple (filled with None if necessary)."""
    tok: list = s.split(sep, maxsplit)
    while len(tok) <= maxsplit:
        tok.append(None)
    return tok


def pop_path(path: str) -> Tuple[str, str]:
    """Return '/a/b/c' -> ('a', '/b/c')."""
    if path in ("", "/"):
        return ("", "")
...
def to_unicode_safe(s: bytes) -> str:
...
# WSGI
# ========================================================================
def get_content_length(scope: HTTPScope):
    """Return a positive CONTENT_LENGTH in a safe way (return 0 otherwise)."""
    # TODO: http://www.wsgi.org/wsgi/WSGI_2.0
    try:
        if not scope.CONTENT_LENGTH:
            return 0

        return max(0, int(scope.CONTENT_LENGTH))
    except ValueError:
        return 0


def read_and_discard_input(scope):
    ...
    the current stream.
    """
    if scope.get("wsgidav.some_input_read") or scope.get("wsgidav.all_input_read"):
        return
    cl = get_content_length(scope)
    assert cl >= 0
    if cl == 0:
        return
...
def make_complete_url(scope: HTTPScope, local_uri=None):
    """URL reconstruction according to PEP 333.
    @see https://www.python.org/dev/peps/pep-3333/#url-reconstruction
    """
    url = scope.url_scheme + "://"

    if scope.HTTP_HOST:
        url += scope.HTTP_HOST
    else:
        url += scope.SERVER_NAME

        if scope.url_scheme == "https":
            if scope.SERVER_PORT != "443":
                url += ":" + scope.SERVER_PORT
        else:
            if scope.SERVER_PORT != "80":
                url += ":" + scope.SERVER_PORT

    url += quote(scope.root_path)

    if local_uri is None:
        url += quote(scope.path, "")
        if scope.query_string:
            url += "?" + to_str(scope.query_string)
    else:
...
async def parse_xml_body(scope: HTTPScope, receive, *, allow_empty=False):
    ...
    try:
        rootEL = etree.fromstring(requestbody)  # type: ignore
    except Exception as e:
        raise DAVError(
...
async def send_status_response(scope, send, e, *, add_headers=None, is_head=False):
    ...
    await send(
        {
            ...
            "status": status,
            "headers": [
                [b"Content-Type", to_bytes(content_type)],
                [b"Date", to_bytes(get_rfc1123_time())],
                [b"Content-Length", to_bytes(str(len(body)))],
            ],
        }
    )
    ...
    await send(
        {
            ...
            "headers": [
                ...
                [b"Date", to_bytes(get_rfc1123_time())],
                [b"Content-Length", to_bytes(str(len(xml_data)))],
            ],
        }
    )
...
def evaluate_http_conditionals(dav_res, last_modified, entity_tag, scope: HTTPScope):
    ...
    if scope.HTTP_IF_MATCH and dav_res.support_etag():
        token_list = parse_if_match_header(scope.HTTP_IF_MATCH)
        match = False
    ...
    if scope.HTTP_IF_MODIFIED_SINCE and dav_res.support_modified():
        ifmodtime = parse_time_string(scope.HTTP_IF_MODIFIED_SINCE)
    ...
    if scope.HTTP_IF_NONE_MATCH and dav_res.support_etag():
        token_list = parse_if_match_header(scope.HTTP_IF_NONE_MATCH)
        for token in token_list:
            if token == entity_tag or token == "*":
                # ETag matched. If it's a GET request and we don't have an
                # conflicting If-Modified header, we return NOT_MODIFIED
                if scope.method in ("GET", "HEAD") and not ifModifiedSinceFailed:
    ...
    if scope.HTTP_IF_UNMODIFIED_SINCE and dav_res.support_modified():
        ifunmodtime = parse_time_string(scope.HTTP_IF_UNMODIFIED_SINCE)
...
def parse_if_header_dict(scope: HTTPScope):
    ...
    """
    if scope.asgidav.conditions._if:
        return

    if not scope.HTTP_IF:
        scope.asgidav.conditions._if = None
        scope.asgidav.ifLockTokenList = []
        return

    iftext = scope.HTTP_IF.strip()
    ...
    scope.asgidav.conditions._if = ifDict
    scope.asgidav.ifLockTokenList = ifLockList
